...
def ex(matches, k):
    ...
    def check_lower(tupla):
        if tupla[0] < tupla[1]:
            ...
            return 1
    def check_lower_sum(tupla,sums):
        if sums[0] < sums[1]:
            return 0
        else:
            return 1
    def check_equal(tupla):
        sums = (sum([ord_values[i] for i in tupla[0]]),sum([ord_values[i] for i in tupla[1]]))
        if sums[0] == sums[1]:
            return check_lower(tupla)
        else:
            return check_lower_sum(tupla,sums)
    def get_string_win(tupla):
        results = [get_char_win(a,b) for a,b in zip(tupla[0],tupla[1]) if a != b]
        results_counts = (results.count(0), results.count(1))
