...
def ex(file_dati, file_png, spaziatura):
    ...
    def get_file_sizes(matrice):
      ...
      file_height = sum([max([palazzo[1] for palazzo in linea]) for linea in matrice]) + (spaziatura * (len(matrice) + 1))
    ...
    def main_v_division(matrice):
        return (len(matrice)-1) if len(matrice) > 1 else 1
    main_vertical_padding = ((image_size[1] - (spaziatura * 2)) - sum(max([building[1] for building in linea]) for linea in matrice)) // main_v_division(matrice)
    ...
    def draw_rectangles(matrice):
      for linea in range(len(matrice)):
        ...
        get_linea_building_width = [matrice[linea][x][0] for x in range(len(matrice[linea][:building]))]
