...
def ex(file_dati, file_png, spaziatura):
    def get_matrice(linea):
      ...
      linea = list(map(lambda x: int(x) if x != '' else x,linea.split(',')))
    ...
    def c_h_division(linea):
        return (linea-1) if linea > 1 else 1
    def solo_case(linea,horizontal_padding):
        return 0 if linea > 1 else (horizontal_padding // 2)
    def get_under_linea_variables(linea):
        # print(sum([building[0] for building in matrice[linea]]))
        horizontal_padding = ((image_size[0] - (spaziatura * 2)) - sum([building[0] for building in matrice[linea]])) // c_h_division(len(matrice[linea]))
        ...
        return horizontal_padding,horizontal_prev_building_sum,linea_building_height_sum,solo_case(len(matrice[linea]),horizontal_padding)

    def get_linea_building_width(linea):
      return [matrice[linea][x][0] for x in range(len(matrice[linea]))]
    def draw_rectangles(matrice):
      for linea in range(len(matrice)):
        ...
        for building in range(len(matrice[linea])):
          ...
          vertical_coordinates = spaziatura+horizontal_padding*building+sum(get_linea_building_width(linea)[:building])+solo_case

          for i in range(building_width):
            for j in range(building_height):
              image_matrix[j+horizontal_coordinates][i+vertical_coordinates] = (matrice[linea][building][2],matrice[linea][building][3],matrice[linea][building][4])
...
if __name__ == '__main__':
    ex("matrices/mat-3-1.txt", 'matrices/mat-3-1.png', 1)
    pass
